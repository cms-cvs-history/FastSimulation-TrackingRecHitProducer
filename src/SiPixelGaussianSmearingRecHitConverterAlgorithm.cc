  ...
  const double boundY)
{
  ...
  if(useCMSSWPixelParameterization) {
    ...
    RectangularPixelTopology rectPixelTopology(theSpecificTopology->nrows(), 
					       ...
					       (theSpecificTopology->nrows()/theSpecificTopology->rocsX()),
					       (theSpecificTopology->ncolumns()/theSpecificTopology->rocsY()));
    //std::cout<<"RectPixTopo from SiPixelGaussianSmearingRecHitConverterAlgo.cc"<<std::endl;
